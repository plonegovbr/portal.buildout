 ...
 * `Projeto Rope <http://rope.sourceforge.net/overview.html>`_
 * `Guia de Estilo do Google <http://google-styleguide.googlecode.com/svn/trunk/pyguide.html>`_
 * `Estilo de Codificação do Pylons <http://docs.pylonsproject.org/en/latest/community/codestyle.html>`_
 * `Tim Pope nas mensagens de commit do Git <http://tbaggery.com/2008/04/19/a-note-about-git-commit-messages.html>`_
 * `Documentações do Plone API <http://docs.plone.org/external/plone.api/docs/contribute/conventions.html>`__
...
.. note::
    ...
    mas arquivos com extensão ``.rst`` ou ``.zcml`` também devem seguir a regra,
    porém não com tanto rigor.
...
2. Não se esqueça de adequar aos 80 caracteres por linha e se necessário
   quebre em mais linhas.
